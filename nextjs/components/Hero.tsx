...
export default function Hero() {
  ...
  const proofs = [
    { k: t("proofs.resolution"), v: t("proofs.resolutionValue") },
    { k: t("proofs.response"),   v: t("proofs.responseValue") },
    { k: t("proofs.uptime"),     v: t("proofs.uptimeValue") },
  ];
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="relative max-w-[1240px] mx-auto px-6 lg:px-10 pt-20 pb-16 lg:pt-28 lg:pb-24 grid lg:grid-cols-[1.1fr_1fr] gap-14 lg:gap-20 items-center">
        ...
        <div className="relative z-10">
          ...
          <div className="bg-white border border-line rounded-xl shadow-lift p-5 relative">
            <div className="flex items-center justify-between pb-4 border-b border-line">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-semibold text-[14px]">{t("demo.agentName")}</div>
            ...
            <div className="mt-4 space-y-3 text-[13.5px]">
              <Bubble side="user">{t("demo.bubble1User")}</Bubble>
              <Bubble side="agent">
                {t("demo.bubble2AgentPre")} <b className="num">{t("demo.bubble2AgentAmount")}</b> {t("demo.bubble2AgentPost")}
              </Bubble>
              <Bubble side="user">{t("demo.bubble3User")}</Bubble>
              <Bubble side="agent">{t("demo.bubble4Agent")}</Bubble>
            </div>

            <div className="mt-4 pt-4 border-t border-line flex items-center justify-between text-[11.5px] text-ink-3">
              <span className="num">{t("demo.footer")}</span>
          ...
          {/* Floating stat card — pinned to logical start */}
          <div className="absolute -bottom-8 start-2 sm:start-6 bg-white border border-line rounded-lg p-4 shadow-lift w-[200px]">
            <div className="text-[11px] font-semibold tracking-wider text-ink-3 uppercase">{t("floatCard.label")}</div>
            <div className="display text-[32px] text-ink num mt-1">{t("floatCard.value")}</div>
            <div className="text-[12px] text-emerald-600 font-semibold flex items-center gap-1 mt-0.5">
              <span>{t("floatCard.delta")}</span>
              <span className="text-ink-3 font-normal">{t("floatCard.vs")}</span>
            </div>
          </div>
        </div>
      </div>
